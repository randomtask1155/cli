...
var _ = Describe("Application Summary Actions", func() {
	...
	Describe("GetApplicationSummaryByNameAndSpace", func() {
		...
		Context("when a route actor is not provided", func() {
			JustBeforeEach(func() {
				summary, warnings, executeErr = actor.GetApplicationSummaryByNameAndSpaceNew(appName, spaceGUID, withObfuscatedValues, nil)
			})
		...
		When("passed with a routeActor", func() {
			...
			JustBeforeEach(func() {
				summary, warnings, executeErr = actor.GetApplicationSummaryByNameAndSpaceNew(appName, spaceGUID, withObfuscatedValues, fakeRouteActor)
			})
